...
// • Triângulos  → neutral-200, overlay, alpha 0.184
// • Arestas     → vermelho puro (220, 0, 0)
// • Pulsos      → vermelho puro (220, 0, 0)
// • Wave ring   → edge exterior FRACTAL, edge interior CIRCULAR, fill BRANCO SÓLIDO
...
const C = {
  ...
  // ── Wave: fill branco sólido, edge exterior fractal ───────────────────────
  WAVE_FILL_COLOR:    '255, 255, 255',   // branco puro
  WAVE_ALPHA_START:   0.6,
  WAVE_ALPHA_END:     1.0,

  // ── Fractal edge exterior ─────────────────────────────────────────────────
  FRACTAL_SAMPLES:    120,   // resolução do contorno
  FRACTAL_ROUGHNESS:  40,    // amplitude da deformação em px
  FRACTAL_OCTAVES:    3,     // oitavas de ruído
...
// ── Fractal noise — multi-octave senoidal ─────────────────────────────────────
const fractalNoise = (angle, time, octaves = C.FRACTAL_OCTAVES) => {
  let noise = 0
  for (let i = 1; i <= octaves; i++) {
    noise += Math.sin(angle * (i * 2.5) + time) * (1 / i)
  }
  return noise
}
...
const NeuralBgDark = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // ── 1. WAVE RINGS ────────────────────────────────────────────────────────
      // Edge exterior → FRACTAL (pontos com ruído senoidal multi-octave)
      // Edge interior → CIRCULAR perfeito
      // Fill          → BRANCO SÓLIDO, opacity 60%→100% do centro para o edge
      ctx.save()
      ...
      s.waves.forEach(w => {
        ...
        // Tempo normalizado para o fractal evoluir com a onda
        const fractalTime = age * 1.5

        ctx.beginPath()

        // ── Edge EXTERIOR — fractal ──────────────────────────────────────────
        const steps = C.FRACTAL_SAMPLES
        for (let i = 0; i <= steps; i++) {
          const angle  = (i / steps) * Math.PI * 2
          const noise  = fractalNoise(angle, fractalTime)
          const rOuter = outerR + noise * C.FRACTAL_ROUGHNESS * (w.forceMult ?? 1)
        ...
        // ── Edge INTERIOR — círculo perfeito (anti-horário = cria buraco) ────
        if (innerR > 0) {
          ctx.moveTo(cx + innerR, cy)
          ctx.arc(cx, cy, innerR, 0, Math.PI * 2, true) // true = anti-horário
        }

        ctx.fillStyle = `rgba(${C.WAVE_FILL_COLOR}, ${waveAlpha})`
        ctx.fill('evenodd')
      })
